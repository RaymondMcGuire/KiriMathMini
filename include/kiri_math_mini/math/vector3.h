...
//!
template <typename T> class Vector<T, 3> final {
public:
  ...
  //! Constructs vector with given parameters s_.
  constexpr Vector(T s_) : x(s_), y(s_), z(s_) {}

  //! Constructs vector with a 2-D vector and a scalar.
  constexpr Vector(const Vector2<T> &v, T z_) : x(v.x), y(v.y), z(z_) {}

  constexpr Vector(const Vector2<T> &v) : x(v.x), y(v.y), z(0) {}

  //! Constructs vector with initializer list.
  template <typename U> Vector(const std::initializer_list<U> &lst);
  ...
  template <typename U> Vector<U, 3> castTo() const;
  ...
  template <typename U> Vector &operator=(const std::initializer_list<U> &lst);
...
template <typename T> Vector3<T> min(const Vector3<T> &a, const Vector3<T> &b);
...
template <typename T> Vector3<T> ceil(const Vector3<T> &a);

//! Returns element-wise floored vector.
template <typename T> Vector3<T> floor(const Vector3<T> &a);
...
template <> constexpr Vector3F zero<Vector3F>() {
  return Vector3F(0.f, 0.f, 0.f);
}

//! Returns double-type zero vector.
template <> constexpr Vector3D zero<Vector3D>() {
  return Vector3D(0.0, 0.0, 0.0);
}

//! Returns the type of the value itself.
template <typename T> struct ScalarType<Vector3<T>> { typedef T value; };
