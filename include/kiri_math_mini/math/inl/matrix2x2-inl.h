/*
 * @Author: Doyub Kim  
 * @Date: 2020-03-20 14:11:30 
 * @Last Modified by: Xu.Wang
 * @Last Modified time: 2020-04-05 21:08:33
 */
#ifndef _KIRI_MATH_DETAIL_MATRIX2X2_INL_H_
#define _KIRI_MATH_DETAIL_MATRIX2X2_INL_H_
...
namespace kiri_math_mini
{
        ...
        const std::initializer_list<std::initializer_list<U>> &lst)
    {
        ...
        KIRI_MATH_ASSERT(width == 2);
        KIRI_MATH_ASSERT(height == 2);
    ...
    {
        return ::kiri_math_mini::absmin(::kiri_math_mini::absmin(_elements[0], _elements[1]),
                                        ::kiri_math_mini::absmin(_elements[2], _elements[3]));
    }

    template <typename T>
    T Matrix<T, 2, 2>::absmax() const
    {
        return ::kiri_math_mini::absmax(::kiri_math_mini::absmax(_elements[0], _elements[1]),
                                        ::kiri_math_mini::absmax(_elements[2], _elements[3]));
    }
    ...
    template <typename T>
    Matrix<T, 2, 2> Matrix<T, 2, 2>::zero()
    {
    ...
    // OpenGL Helper
    template <typename T>
    Matrix<T, 2, 2> Matrix<T, 2, 2>::scaleMatrix(T sx, T sy)
    {
        return Matrix(sx, 0, 0, sy);
    }

    template <typename T>
    Matrix<T, 2, 2> Matrix<T, 2, 2>::scaleMatrix(const Vector<T, 2> &s)
    {
        return scaleMatrix(s.x, s.y);
    }

    template <typename T>
    Matrix<T, 2, 2> Matrix<T, 2, 2>::rotationMatrix(const T &rad)
    {
...
#endif // _KIRI_MATH_DETAIL_MATRIX2X2_INL_H_
