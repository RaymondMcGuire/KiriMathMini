...
namespace kiri_math_mini
{
  ...
  template <typename T>
  constexpr size_t Vector<T, 3>::size() const { return 3; }
  ...
  {
    return kiri_math_mini::absmin(kiri_math_mini::absmin(x, y), z);
  }

  template <typename T>
  T Vector<T, 3>::absmax() const
  {
    return kiri_math_mini::absmax(kiri_math_mini::absmax(x, y), z);
  }
