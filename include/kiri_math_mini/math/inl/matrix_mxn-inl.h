...
#ifndef _KIRI_MATH_DETAIL_MATRIX_MXN_INL_H_
#define _KIRI_MATH_DETAIL_MATRIX_MXN_INL_H_
...
namespace kiri_math_mini
{
    ...
    void MatrixMxN<T>::setRow(size_t i, const VectorExpression<T, E> &row)
    {
        ...
        const E &e = row();
        parallelFor(kZeroSize, cols(), [&](size_t j)
                    { (*this)(i, j) = e[j]; });
    }
    ...
    void MatrixMxN<T>::setColumn(size_t j, const VectorExpression<T, E> &col)
    {
        ...
        const E &e = col();
        parallelFor(kZeroSize, rows(), [&](size_t i)
                    { (*this)(i, j) = e[i]; });
    }
...
#endif // _KIRI_MATH_DETAIL_MATRIX_MXN_INL_H_
