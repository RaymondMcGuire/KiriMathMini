/*
 * @Author: Doyub Kim  
 * @Date: 2020-03-20 14:10:24 
 * @Last Modified by: Xu.Wang
 * @Last Modified time: 2020-04-05 20:51:33
 */
#ifndef _KIRI_MATH_DETAIL_MATRIX3X3_INL_H_
#define _KIRI_MATH_DETAIL_MATRIX3X3_INL_H_
...
namespace kiri_math_mini
{
    ...
    // OpenGL Helper
    template <typename T>
    Matrix<T, 3, 3> Matrix<T, 3, 3>::scaleMatrix(T sx, T sy, T sz)
    {
        return Matrix(sx, 0, 0, 0, sy, 0, 0, 0, sz);
    }

    template <typename T>
    Matrix<T, 3, 3> Matrix<T, 3, 3>::scaleMatrix(const Vector<T, 3> &s)
    {
        return scaleMatrix(s.x, s.y, s.z);
    }

    template <typename T>
    Matrix<T, 3, 3> Matrix<T, 3, 3>::rotationMatrix(const Vector<T, 3> &axis,
                                                    T rad)
    {
        return Matrix(
...
#endif // _KIRI_MATH_DETAIL_MATRIX3X3_INL_H_
