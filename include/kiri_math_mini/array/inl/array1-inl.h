 ...
 * @Last Modified time: 2019-12-13 22:43:30
 */
#ifndef _KIRI_MATH_DETAIL_ARRAY1_INL_H_
#define _KIRI_MATH_DETAIL_ARRAY1_INL_H_
...
namespace kiri_math_mini
{
    ...
    template <typename T>
    void Array<T, 1>::remove(size_t i)
    {
        _data.erase(_data.begin() + i);
    }

    template <typename T>
    void Array<T, 1>::remove(size_t start, size_t end)
    {
        _data.erase(_data.begin() + start, _data.begin() + end + 1);
    }

    template <typename T>
    void Array<T, 1>::removeq(size_t i)
    {
        _data[i] = _data.back();
        _data.pop_back();
    }
...
#endif // _KIRI_MATH_DETAIL_ARRAY1_INL_H_
