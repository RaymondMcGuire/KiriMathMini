#ifndef _KIRI_MATH_DETAIL_PDE_INL_H_
#define _KIRI_MATH_DETAIL_PDE_INL_H_
...
namespace kiri_math_mini
{
    ...
    std::array<T, 2> weno5(T *v, T h, T eps)
    {
        ...
        for (int K = 0; K < 2; ++K)
        {
            ...
            T s1 = c_13_12 * square(vdev[0] - 2 * vdev[1] + vdev[2]) + c_1_4 * square(vdev[0] - 4 * vdev[1] + 3 * vdev[2]);
            T s2 = c_13_12 * square(vdev[1] - 2 * vdev[2] + vdev[3]) + c_1_4 * square(vdev[1] - vdev[3]);
            T s3 = c_13_12 * square(vdev[2] - 2 * vdev[3] + vdev[4]) + c_1_4 * square(3 * vdev[2] - 4 * vdev[3] + vdev[4]);
...
#endif // _KIRI_MATH_DETAIL_PDE_INL_H_
